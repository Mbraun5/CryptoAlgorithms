...
class TestAlgorithms(unittest.TestCase):
    ...
    def test_monoalphabetic(self):
        testdir = os.path.join(os.path.dirname(os.path.realpath(__file__)), "Monoalphabetic")
        for file in os.listdir(testdir):
            with self.subTest(file=file):
                if file.startswith("dec_"):
                    continue
                    ...
                    Main.test_integration("MAC", file[:len(file)-4], "{}/{}".format(testdir, file), "test_enc.txt", "ENC")
                    Main.test_integration("MAC", file[:len(file)-4], "test_enc.txt", "test_dec.txt", "DEC")
                    self.assertTrue(filecmp.cmp("{}/enc_{}".format(testdir, file), "test_enc.txt", shallow=False))
                    self.assertTrue(filecmp.cmp("{}/dec_{}".format(testdir, file), "test_dec.txt", shallow=False))
                    os.remove("test_enc.txt")
                    os.remove("test_dec.txt")
